// ==========================================
// 1. INJECT DEPENDENCIES & STYLES (REPEAT PDF)
// ==========================================
(function initEnvironment() {
    // 1. pdf-lib for processing/duplicating the file
    if (!window.PDFLib) {
    ...
    style.innerHTML = `
        ...
        .repeat-config-wrapper { display: none; flex-direction: column; width: 100%; max-width: 600px; margin: 0 auto 3rem auto; animation: fadeIn 0.4s ease; }
        ...
        .input-number { background: var(--bg-base); border: 1px solid rgba(0, 255, 204, 0.3); color: #fff; padding: 0.6rem 1rem; border-radius: 6px; font-family: 'JetBrains Mono', monospace; font-size: 1.1rem; width: 100px; text-align: center; outline: none; transition: all 0.3s; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    configWrapper.className = 'repeat-config-wrapper';
    configWrapper.innerHTML = `
        <div class="config-box">
            ...
            <div class="config-row">
                <label class="config-label">How many times to repeat?</label>
                <input type="number" id="repeat-count" class="input-number" min="2" max="1000" value="2">
            </div>
        </div>
        <div class="action-container" id="action-container" style="margin-top: 1.5rem;">
            <button class="btn-action" id="btn-execute-repeat"><i class="fa-solid fa-repeat"></i> Repeat PDF</button>
        </div>
    `;
    
    dropzone.parentNode.insertBefore(configWrapper, dropzone.nextSibling);

    const btnExecuteRepeat = configWrapper.querySelector('#btn-execute-repeat');
    btnExecuteRepeat.addEventListener('click', executeRepeat);
    ...
    // Render a single thumbnail of page 1 to represent the file
    async function renderPreviewThumbnails() {
    ...
    // ==========================================
    // CLIENT-SIDE REPEAT LOGIC
    // ==========================================
    async function executeRepeat() {
        if (!safePdfBytes) return;

        const countInput = document.getElementById('repeat-count');
        let times = parseInt(countInput.value);

        if (isNaN(times) || times < 1) times = 1;
        if (times > 1000) { alert("Maximum 1000 repetitions allowed at once."); times = 1000; countInput.value = 1000; }

        try {
            btnExecuteRepeat.disabled = true;
            btnExecuteRepeat.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Duplicating Pages...';
            ...
            // Loop and copy all pages sequence by sequence
            ...
            const finalFileName = `PDFase_${baseName}_Repeated.pdf`;
            const totalResultPages = totalPages * times;
            ...
            setTimeout(() => {
                ...
                finalContainer.innerHTML = `
                    <div class="success-message">
                        <i class="fa-solid fa-circle-check"></i> Repetition Complete!
                    </div>
                    <div class="file-flow">
                        <i class="fa-solid fa-repeat" style="color: var(--cyber-cyan); font-size: 1.2rem;"></i>
                        <span>Repeated <strong>${times} times</strong> (${totalResultPages} total pages) inside <strong>${finalFileName}</strong></span>
                    </div>
                    <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                        ...
                        <button class="btn-secondary" onclick="resetTool()">
                            <i class="fa-solid fa-rotate-right"></i> Repeat Another
                        </button>
            ...
            console.error('Repeating Error:', error);
            alert('A critical error occurred while processing. Error: ' + error.message);
            btnExecuteRepeat.disabled = false;
            btnExecuteRepeat.innerHTML = '<i class="fa-solid fa-repeat"></i> Repeat PDF';
        }
    }
