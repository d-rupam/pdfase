...
(function initEnvironment() {
    // Inject PDF.js for rendering PDF pages to Canvas
    if (!window.pdfjsLib) {
    ...
    // Inject JSZip for packaging multiple images into a downloadable zip
    if (!window.JSZip) {
    ...
    // Inject Canvas-to-TIFF encoder since browsers do not natively support canvas.toBlob('image/tiff')
    if (!window.CanvasToTIFF) {
        const scriptTiff = document.createElement('script');
        scriptTiff.src = 'https://cdn.jsdelivr.net/npm/canvas-to-tiff@1.2.2/canvas-to-tiff.min.js';
        document.head.appendChild(scriptTiff);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initConvertUI() {
        ...
        convertBtn.innerHTML = '<i class="fa-regular fa-image"></i> Convert to TIFF';
    ...
    // ==========================================
    // 5. CLIENT-SIDE CONVERSION LOGIC (pdf.js + canvas-to-tiff + jszip)
    // ==========================================
    async function executeConversion() {
        ...
        if (!window.pdfjsLib || !window.JSZip || !window.CanvasToTIFF) {
        ...
        try {
            ...
            // 3. Render each page to Canvas, then to TIFF Blob
            for (let i = 1; i <= totalPages; i++) {
                ...
                // Fill with white background (TIFF typically requires opaque background for document storage)
                ...
                // Convert canvas to TIFF blob using the injected library
                const blob = await new Promise((resolve, reject) => {
                    window.CanvasToTIFF.toBlob(canvas, function(tiffBlob) {
                        if (tiffBlob) resolve(tiffBlob);
                        else reject(new Error("TIFF encoding failed"));
                    });
                });
                ...
                zip.file(`Page_${paddedNumber}.tiff`, blob);
            }
            ...
            const finalFileName = `PDFase_${baseName}_TIFFs.zip`;
                ...
                actionContainer.innerHTML = `
                    ...
                    <div class="file-flow">
                        ...
                        <span style="display: block; width: 100%; margin-top: 5px; color: var(--text-muted); font-size: 0.8rem;">
                            Extracted ${totalPages} high-res TIFF(s)
                        </span>
                    </div>
