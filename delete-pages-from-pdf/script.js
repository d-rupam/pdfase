// ==========================================
// 1. INJECT DEPENDENCIES & STYLES (DELETE PDF PAGES)
// ==========================================
(function initEnvironment() {
    // 1. pdf-lib for processing/deleting the actual file
    if (!window.PDFLib) {
    ...
    style.innerHTML = `
        ...
        /* Delete Styling */
        .page-card.selected { border-color: #ff3366; transform: translateY(-3px); box-shadow: 0 6px 20px rgba(255, 51, 102, 0.25); opacity: 0.6; }
        
        .page-card canvas { width: 100%; height: 100%; object-fit: contain; pointer-events: none; transition: opacity 0.3s; }
        .page-card.selected canvas { filter: grayscale(100%); opacity: 0.5; }
        
        /* Loading spinner inside card */
        .card-loader { color: var(--bg-card); font-size: 1.5rem; position: absolute; }

        /* Page Number Badge overlay */
        .page-badge { position: absolute; bottom: 4px; right: 4px; background: rgba(0,0,0,0.7); color: #fff; font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; padding: 2px 6px; border-radius: 4px; z-index: 5; backdrop-filter: blur(2px); pointer-events: none; transition: background 0.3s; }
        .page-card.selected .page-badge { background: #ff3366; color: #fff; font-weight: bold; }
        
        /* Selection Trash Icon */
        .check-icon { position: absolute; top: 4px; left: 4px; background: #ff3366; color: #fff; border-radius: 50%; width: 26px; height: 26px; font-size: 0.8rem; display: none; justify-content: center; align-items: center; box-shadow: 0 2px 8px rgba(255, 51, 102, 0.5); z-index: 10; pointer-events: none; }
        ...
        .btn-action { background-color: #ff3366; color: #fff; border: none; padding: 0.85rem 2.5rem; font-size: 1.05rem; font-weight: 700; font-family: 'Space Grotesk', sans-serif; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 0 15px rgba(255, 51, 102, 0.2); text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
        .btn-action:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 5px 20px rgba(255, 51, 102, 0.4); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initDeleteUI() {
        ...
        deleteBtn.innerHTML = '<i class="fa-solid fa-trash"></i> Delete Pages';
    ...
    async function renderAllCanvases() {
        for (let i = 1; i <= totalPages; i++) {
            ...
            card.innerHTML = `
                <div class="check-icon"><i class="fa-solid fa-trash"></i></div>
    ...
    function updateActionState() {
        ...
        if (count === 0) {
            ...
            deleteBtn.innerHTML = `<i class="fa-solid fa-trash"></i> Delete ${count} Page${count > 1 ? 's' : ''}`;
        }
    }
    ...
    // ==========================================
    // 6. CLIENT-SIDE DELETE LOGIC (pdf-lib)
    // ==========================================
    async function executeDelete() {
        if (selectedPages.size === 0 || !safePdfBytes) return;

        // Safety check: Cannot delete all pages
        if (selectedPages.size === totalPages) {
            alert("You cannot delete every page! A PDF document must have at least one page remaining.");
            return;
        }

        try {
            deleteBtn.disabled = true;
            deleteBtn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Removing Pages...';
            ...
            // INVERSE LOGIC: We copy only the pages that are NOT selected for deletion
            const pagesToKeep = [];
            for (let i = 1; i <= totalPages; i++) {
                if (!selectedPages.has(i)) {
                    pagesToKeep.push(i - 1); // 0-indexed for pdf-lib
                }
            }
            ...
            setTimeout(() => {
                ...
                actionContainer.innerHTML = `
                    <div class="success-message">
                        <i class="fa-solid fa-circle-check"></i> Removal Complete!
                    </div>
                    <div class="file-flow">
                        <span class="file-flow-name">${currentFileName}</span>
                        <i class="fa-solid fa-trash" style="color: #ff3366; margin: 0 10px; font-size: 0.8rem;"></i>
                        <span class="file-flow-name">${selectedPages.size} pages deleted</span>
                    ...
                    <div class="button-group">
                        <a href="${url}" download="${finalFileName}" class="btn-action" style="background-color: var(--cyber-cyan); color: #000;">
                            <i class="fa-solid fa-download"></i> Download PDF
                        </a>
                        <button class="btn-secondary" onclick="resetTool()">
                            <i class="fa-solid fa-rotate-right"></i> Delete Another
                        </button>
