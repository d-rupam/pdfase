// ==========================================
// 1. INJECT DEPENDENCIES & STYLES (REVERSE PDF)
// ==========================================
(function initEnvironment() {
    // 1. pdf-lib for processing/reversing the actual file
    if (!window.PDFLib) {
    ...
    style.innerHTML = `
        ...
        .controls-container { display: none; justify-content: center; align-items: center; width: 100%; margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-subtle); flex-wrap: wrap; gap: 10px; }
        ...
        /* Canvas Page Cards (Read-only preview) */
        .page-card { 
            width: 130px; 
            height: 184px; 
        ...
        .page-card:hover { border-color: rgba(0, 255, 204, 0.5); transform: translateY(-3px); }
        ...
        .page-badge { position: absolute; bottom: 4px; right: 4px; background: rgba(0,0,0,0.7); color: #fff; font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; padding: 2px 6px; border-radius: 4px; z-index: 5; backdrop-filter: blur(2px); pointer-events: none; border: 1px solid rgba(255,255,255,0.1); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initReverseUI() {
        ...
        reverseBtn.className = 'btn-action';
        reverseBtn.innerHTML = '<i class="fa-solid fa-clock-rotate-left"></i> Reverse PDF';
        reverseBtn.addEventListener('click', executeReverse);
        
        btnGroup.appendChild(reverseBtn);
    ...
    // ==========================================
    // 5. CLIENT-SIDE REVERSE LOGIC (pdf-lib)
    // ==========================================
    async function executeReverse() {
        ...
        try {
            ...
            reverseBtn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Reversing Document...';
            ...
            // Mathematically invert the page indices
            const reversedIndices = [];
            for (let i = totalPages - 1; i >= 0; i--) {
                reversedIndices.push(i);
            }

            const copiedPages = await newPdf.copyPages(originalPdf, reversedIndices);
            ...
            const finalFileName = `PDFase_${baseName}_Reversed.pdf`;
            ...
            setTimeout(() => {
                ...
                actionContainer.innerHTML = `
                    <div class="success-message">
                        <i class="fa-solid fa-circle-check"></i> Reversal Complete!
                    </div>
                    <div class="file-flow">
                        <span class="file-flow-name">${currentFileName}</span>
                        <i class="fa-solid fa-clock-rotate-left" style="color: var(--cyber-cyan); margin: 0 10px; font-size: 0.8rem;"></i>
                        <span class="file-flow-name">Flipped ${totalPages} Pages</span>
                    ...
                    <div class="button-group">
                        ...
                        <button class="btn-secondary" onclick="resetTool()">
                            <i class="fa-solid fa-rotate-right"></i> Reverse Another
                        </button>
