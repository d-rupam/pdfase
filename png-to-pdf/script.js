// ==========================================
// 1. INJECT DEPENDENCIES & STYLES (PNG TO PDF)
// ==========================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    dropzone.addEventListener('click', (e) => {
        // If empty, clicking anywhere opens the file picker
    ...
    function handleFiles(files) {
        // Strict filter for PNG files only
        const newFiles = Array.from(files).filter(file => 
            file.type === 'image/png' || file.name.toLowerCase().endsWith('.png')
        );
        
        if (newFiles.length === 0) {
            alert('No valid PNG images found. Please select only PNG files.');
            return;
        }
        ...
        activeImageFiles.forEach((file, index) => {
            ...
            item.innerHTML = `
                ...
                <div class="a4-icon-wrapper">
                    <img src="${objectUrl}" class="a4-thumbnail" alt="${file.name}">
                </div>
    ...
    // Helper to read file as Data URL for jsPDF
    ...
    async function executeConversion() {
        if (activeImageFiles.length === 0) {
            alert('Please add at least one PNG image.');
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < activeImageFiles.length; i++) {
                ...
                // Specifically state format as 'PNG' to ensure losslessness in jsPDF
            ...
            setTimeout(() => {
                ...
                actionContainer.innerHTML = `
                    ...
                    <div class="file-flow">
                        <span class="file-flow-name">${activeImageFiles.length} Image(s)</span>
            ...
            alert('An error occurred while building the PDF. Ensure all files are valid PNGs.');
