// ==========================================
// 1. INJECT DEPENDENCIES & STYLES (INVERT PDF COLORS)
// ==========================================
(function initEnvironment() {
    ...
    style.innerHTML = `
        ...
        .a4-icon { font-size: 2.2rem; color: var(--theme-color, #39ff14); transition: color 0.2s; }
        ...
        .options-panel { background: rgba(57, 255, 20, 0.02); border: 1px solid rgba(57, 255, 20, 0.15); padding: 1.25rem 1.5rem; border-radius: 8px; display: flex; flex-direction: column; gap: 0.75rem; align-items: center; width: 100%; max-width: 450px; font-family: 'Space Grotesk', sans-serif; text-align: center; }
        .options-panel p { color: var(--text-muted); font-size: 0.9rem; line-height: 1.5; margin: 0; }

        /* Progress Bar UI */
        .progress-wrapper { width: 100%; display: none; flex-direction: column; gap: 8px; margin-top: 5px; }
        ...
        .progress-fill { height: 100%; width: 0%; background: var(--theme-color, #39ff14); transition: width 0.2s ease; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initInvertUI() {
        actionContainer.innerHTML = '';
        
        const optionsPanel = document.createElement('div');
        optionsPanel.className = 'options-panel';
        optionsPanel.innerHTML = `
            <p><i class="fa-solid fa-circle-half-stroke" style="color: var(--theme-color);"></i> Ready to render and invert PDF pages into eye-friendly dark mode vectors.</p>
            
            <div class="progress-wrapper" id="invert-progress-wrapper">
                <div class="progress-text" id="invert-progress-text">Preparing Engine...</div>
                <div class="progress-track">
                    <div class="progress-fill" id="invert-progress-fill"></div>
                </div>
            </div>
        `;
        ...
        actionBtn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Invert Colors';
        actionBtn.addEventListener('click', executeColorInversion);
    ...
    // ==========================================
    // 4. CLIENT-SIDE COLOR INVERSION LOGIC
    // ==========================================
    async function executeColorInversion() {
        ...
        const progressWrapper = document.getElementById('invert-progress-wrapper');
        const progressText = document.getElementById('invert-progress-text');
        const progressFill = document.getElementById('invert-progress-fill');

        try {
            actionBtn.disabled = true;
            actionBtn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Inverting Colors...';
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                const viewport = page.getViewport({ scale: 2.0 }); // High res rendering for crisp text
                ...
                // Invert pixel colors directly on canvas context data
            ...
            progressText.innerText = "Compiling Dark Mode PDF...";
            const pdfBytes = await newPdf.save();
            const blob = new Blob([pdfBytes], { type: 'application/pdf' });
            const url = URL.createObjectURL(blob);
            
            const baseName = mainPdfFile.name.replace(/\.[^/.]+$/, "");
            const finalFileName = `PDFase_${baseName}_DarkMode.pdf`;
            ...
            setTimeout(() => {
                ...
                actionContainer.innerHTML = `
                    <div class="success-message">
                        <i class="fa-solid fa-circle-check"></i> Colors Inverted Successfully!
                    </div>
                    <div class="file-flow">
                    ...
                    <div class="button-group" style="margin-top: 5px;">
                        <a href="${url}" download="${finalFileName}" class="btn-action">
                            <i class="fa-solid fa-download"></i> Download Dark Mode PDF
                        </a>
                        <button type="button" class="btn-secondary" onclick="resetTool()">
                            <i class="fa-solid fa-rotate-right"></i> Invert Another
                        </button>
                    </div>
                    ${stealthAttribution}
                `;
            ...
            alert('A critical error occurred while inverting colors on the PDF document.');
